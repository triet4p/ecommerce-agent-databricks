...
print("=== CONVERSATIONS ===")
cur.execute("SELECT id, owner, title, created_at FROM conversations ORDER BY created_at DESC")
for r in cur.fetchall():
    print(f"  {r[0]}: owner={r[1]} title='{r[2]}' created={r[3]}")

print("\n=== TURNS ===")
cur.execute("SELECT id, conversation_id, sequence, status, mlflow_trace_id FROM turns ORDER BY sequence ASC")
for r in cur.fetchall():
