"""Read-only Lakebase schema and replay verification.
...
conn = psycopg.connect(
    host=host,
    user=cred.get("username") or os.environ.get("PGUSER"),
    password=token,
...
if not all_tables:
    raise SystemExit(
        "Conversation schema is absent; only the App service principal may run migrations"
    )
else:
        ...
        json.dumps(
            {
                ...
                "role": "user",
                "content": [
                    {"type": "input_text", "text": "Where is my order #12345?"}
                ],
            }
        ),
    ),
        ...
        json.dumps(
            {
                ...
                "content": [
                    {
                        "type": "output_text",
                        "text": "Your order #12345 has been shipped and is expected to arrive by July 22nd.",
                    }
                ],
            }
        ),
        ...
        json.dumps(
            {
                ...
                "content": [
                    {
                        "type": "input_text",
                        "text": "Thanks! Can you update my shipping address?",
                    }
                ],
            }
        ),
...
for row in all_rows:
    items.append(
        ConversationItem(
            ...
            role=row[5],
            payload=ItemPayload(
                **row[6] if isinstance(row[6], dict) else json.loads(row[6])
            ),
            item_key=row[7],
            created_at=row[8],
        )
...
print(
    "[PASS] History replay includes cross-turn context (turn 1 context visible in turn 3)"
)
